...
public class ShoppingServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if("addCar".equals(action)){
			...
			int reg=shoppingCar.add(product,Integer.parseInt(num));
			session.setAttribute("shoppingCar", shoppingCar);
			out.print(reg);
		}else if("jianCar".equals(action)){
			String num=request.getParameter("num");
			ShoppingCar shoppingCar=null;
			HttpSession session=request.getSession();
			if(session.getAttribute("shoppingCar")!=null){
			...
			ProductService productService=new ProductServiceImpl();
			Product product=productService.selByid(Integer.parseInt(id));
			int reg=shoppingCar.jian(product,Integer.parseInt(num));
			session.setAttribute("shoppingCar", shoppingCar);
			out.print(reg);
		}else if("addSho".equals(action)){
